...
PARAGRAPH HERE 1) look at signal in the basal ganglia relative to the cortex - and so identified key nodes using a meta-analytic approach
2) as it remains relatively theoretical/hypthetical (the influence of the human basal ganglia in visual selection), compare signal in the basal ganglia between when modeling visual spatial attention shifts vs overt action
3) even though we have used a meta-analytic approach to identify cortical nodes (references), we know there may be differences between 3T and 7T localisation (as meta-analysis is based on 3T), and individual differences in cortical recruitment/localisation - therefore compare snr from key nodes of interest relative to most responsive voxels across the brain (i.e thresholded).
...
-> add bit about motivation for the current focus of the analysis
In the current work we focus investigate SNR in the basal ganglia in response to probabilistic spatial cues for the following reasons; the influence of spatial cues on visual-selection has been documented over multiple decades (e.g. @posnerOrientingAttention1980, @carrascoVisualAttention252011, @chicaSpatialOrientingParadigm2014), particularly in relation to corresponding cortical activity [@corbettaControlGoaldirectedStimulusdriven2002, @desimoneNeuralMechanismsSelective1995, @fiebelkornDynamicInterplayFrontoparietal2018], and have also been the subject of recent scrutiny with regard to the contribution of basal ganglia/cortical interactions [@wangInvolvementStriatalDirect2020, @arcizetCovertSpatialSelection2018, @hermanAttentionrelatedModulationCaudate2020]. 
...
-> Possibly add some of this here?

We used convergent theoretical and meta-analytic approaches to identify the key regions of interest pertinent to the current work. As mentioned, there is an increasing body of evidence asserting a role for the basal ganglia in the performance of visual selection behaviours [@arcizetCovertSpatialSelection2018, @amitaOptogeneticManipulationValuecoding2020a, yamamotoWhatWhereInformation2012]. Moreover, theoretical models of basal ganglia function assert its role as a compressor of salient sensory inputs, relaying signals to the frontal cortex to assist in the planning of subsequent actions [bar-gadInformationProcessingDimensionality2003], or as a general resource selection mechanism that arbitrates between competing sensory inputs in order to regulate access to motor output functions [@gurneyComputationalModelAction2001]. It is easy to imagine how both of these mechanisms could support visual selection behaviours, where sensory inputs are used to guide future overt and covert attention shifts. Moreover, a recent model posits that a closed-loop circuit comprising the caudate tail, the posterior putamen, and superior colliculus serves to manage visual selection for well-learned sensory-motor associations [@espositoSubcorticalNetworkImplicit2021], such as when a probabilistic spatial cue serves to indicate a likely target location. We therefore examine SNR in each of the nuclei of the basal ganglia, specifically the caudate nucleus (CN), putamen (Put), ventral striatum (VS), globus pallidus external (GPe) and internal (GPi), the subthalamic nucleus (STN) and the nucleus accumbens (NAcc) (see Figure \@ref{fig:aims}). To identify the cortical regions of current interest (where anatomically defined boundaries are not so readily defined), we used a [neurosynth](https://neurosynth.org/) meta-analysis to find functional associations to visual selection behaviours, particularly in response to spatial-cues. This analysis (described below) yielded the frontal eye fields (FEF), lateral occipital cortex (LOC) and the intra-parietal sulcus (IPS).  
...
Next we sought to determine which protocol yielded the highest CNR values, at the voxels that showed the strongest activations to the experimental events of interest. To attain responsive voxels, each contrast image was thresholded at p<.001. First we examined the CNR values for each response hand (left or right > 0), by applying a LME model to the response hand $t$-scores, with contrast (left or right hand), protocol and their interaction modelled as fixed effects, and with an intercept term for each subject.
...
The main effect and pattern of protocol differences was consistent across the remaining analyses, whether CNR data were computed using the target laterality (left or right) contrasts (F(`r thrshmod_TCT_an["TR", "Df"]`, `r thrshmod_TCT_an["TR", "Df.res"]`) = `r thrshmod_TCT_an["TR", "F"]`, p = `r thrshmod_TCT_an["TR", "Pr(>F)"]`), the spatial cue probability contrasts (p=.8 or p=.5, F(`r thrshmod_pspatial_an["TR", "Df"]`, `r thrshmod_pspatial_an["TR", "Df.res"]`) = `r thrshmod_pspatial_an["TR", "F"]`, p = `r thrshmod_pspatial_an["TR", "Pr(>F)"]`) or the target laterality|p = .8 contrasts (F(`r thrshmod_lat_pspatial_an["TR", "Df"]`, `r thrshmod_lat_pspatial_an["TR", "Df.res"]`) = `r thrshmod_lat_pspatial_an["TR", "F"]`, p = `r thrshmod_lat_pspatial_an["TR", "Pr(>F)"]`). Also consistent was the absence of statistically significant influences of contrast, or any contrast x protocol interactions (all ps > `r min(thrsh_CNR_ps)`). Therefore, across all modelled experimental events of interest, the ME protocol fared best, followed by the CMRR protocol, which in turn outperformed the 3D EPI. 
...
# Discussion
- reason not all cortical ROIs showed a significant influence of protocol.
