...
Summary of results - namely, that ME and CMRR showed CNR advantage over the 3D-EPI for CNR, particularly for comparisons between experimental events of interest.

First conclusion is that ME or CMRR both show advantages when looking at modeling visual attention function events. This was not the case when modelling overt responses. Therefore it may be that once events become sufficiently subtle - such as psychological manipulations, there may be an upper bound on activation, and good sequences turn out comparable.

It appears that in no case did we see basal ganglia signal increases, even for overt action. Describe how action used here differs from that in Puckett and Forstmann. Therefore, all signals were equal at this point, so maybe use the one that is simplest.

Although there is strong theoretical reason to believe that the basal ganglia nuclei contribute to visual selection, it is not readily visible with 7T. Future efforts may seek to be more precise than the anatomical region - e.g. performing a functional localiser combined with ROI approach (any examples in the literature?).

Reason the results varied for the cortical ROIs - differences between 3T and 7T. Also it may be that individual variation is too large to apply a meta-analytic approach/define cortical ROIs based on localisation of function. Future efforts may wish to use a cortical parcellation scheme instead.
