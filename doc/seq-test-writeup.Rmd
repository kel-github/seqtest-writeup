...
roi_act$TR <- roi_act$TR %>% recode('700' = 'ME',
                                    '1510' = 'CMRR',
                                    '1920' = '3D EPI')
mutConts <- tconts_thrsh %>% group_by(sub, TR, contrast) %>%
                            summarise(mu = mean(CNR)) %>% ungroup()
rm(tmp)
```

```{r}
tmp <- lmer( CNR ~ TR*contrast*q*roi + (1|sub), data = roi_act %>% filter(contrast == "lft_hnd" | contrast == "rght_hnd"))
Anova(tmp, test.statistic="F")
