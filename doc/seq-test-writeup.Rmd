...
The tSNR data were extracted for each subject, protocol and ROI, the results of which are presented in Figure \@ref{fig:tSNR}. As can be seen from the example whole brain tSNR map (panel A), strongest tSNR was observed for the ME protocol, then for the 3D EPI, and last for the CMRR protocol. 
...
## Comparison of CNR between protocols
\label{sec:tSNRResults}

Comparison of CNR between protocols occurred over the following steps: first we sought to compare CNR values between protocols, using voxels from across the whole brain that were responsive to the manipulation of interest. This provides an understanding of how the protocols fare where the signal is strongest, and thus provides a basis for comparison when subsequently comparing CNR values from the _a priori_ defined ROIs, which were derived theoretically and from meta-analyses, rather than from an examination of the current signal.

### Thresholded voxels
\label{sec:thrshVox}

We first applied a threshold of p< .001 to each contrast image (response hand, target laterality, cue probability and laterality x cue probability interaction), and computed the average CNR value from the remaining voxels. The resulting images from a representative participant are shown in Figure \@ref{fig:}, for the response hand contrast (panel A) and the target laterality x cue probability interaction (panel C). In accordance with the tSNR analysis, there was a statistically significant main effect of protocol (F(`r thrsh_lme_an["TR", "Df"]`, `r thrsh_lme_an["TR", "Df.res"]`) = `r thrsh_lme_an["TR", "F"]`, p = `r thrsh_lme_an["TR", "Pr(>F)"`]). 
